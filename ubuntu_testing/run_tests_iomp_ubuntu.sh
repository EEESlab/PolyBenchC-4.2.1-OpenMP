#!/bin/bash
# =============================================================================
# run_tests_opt_par_only.sh — MLIR OpenMP correctness check (PARALLEL ONLY, IOMP)
# + saves final LLVM IR (.ll)
#
# Compiles each kernel two ways and diffs the array dumps:
#   ref — clang -O3 -fopenmp       (libomp / IOMP, strict FP flags)
#   opt — CIR/MLIR pipeline        (iomp runtime,  strict FP flags)
#
# Strict FP flags (-ffp-contract=off + no auto-vectorisation) are required for
# bit-identical results between clang and the CIR/MLIR pipeline. Without them,
...
# OpenMP headers — adjust if your system uses a different GCC version
...
compile_opt_par() {
    ...
    # Step 3: Custom OMP lowering (runtime = iomp / libomp)
    mlir-opt-omp \
    ...
    # Step 9: Link (strict FP, IOMP = libomp)
    clang -O3 ${CLANG_STRICT_FP} \
        -fopenmp -no-pie \
...
compile_ref() {
    ...
    # ref uses clang -fopenmp → libomp (IOMP), same FP flags as opt
    clang -O3 ${CLANG_STRICT_FP} -fopenmp \
        -I"$INC" -I"$(dirname "$src")" -I"$INC_OMP" \
        -D${DATASET} ${POLYBENCH_CFLAGS} \
        "$src" "$INC/polybench.c" \
        -lm -o "$outdir/${binname}"
...
run_kernel() {
    ...
    # ── Compile ─────────────────────────────
    echo "  [1/4] compiling ref (clang -fopenmp / IOMP, strict FP)..."
    ...
    echo "  [2/4] compiling opt (MLIR, iomp runtime, strict FP)..."
...
echo "=== MLIR OpenMP CORRECTNESS CHECK (${DATASET}) — IOMP runtime ==="
...
if [ "$failed" -gt 0 ] || [ "$errors" -gt 0 ]; then
    ...
    grep -E ';(FAIL|ERROR)$' results_correctness.csv | sed 's/^/  /'
fi
